---
description: Reset the entire IRIS repo to a clean state
allowed-tools: Bash, Read, Edit
---

Reset the IRIS repository to a clean state by deleting all projects and restoring default configs.

**This is extremely destructive.** Before doing anything:

1. List what will be destroyed:
   - Run `iris project list` to show all projects and their output/reference counts.
   - Run `iris config show` to show current config state.
2. Print a clear warning:
   ...
   WARNING: This will permanently destroy:
     ...
     - The active project pointer (.iris/active_project)
...
4. Only after confirmation, execute the reset:
   a. Run `iris project close` to clear the active pointer.
