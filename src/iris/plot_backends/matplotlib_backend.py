"""Matplotlib plot handlers for IRIS.
...
from iris.plot_backends._common import (
    _show_params_panel,
    _time_axis_ms,
    _window_suffix,
)
...
def plot_mea_trace(result: MEATrace, ctx: PipelineContext, last_op) -> None:
    ...
    print(
        f"  [plot_mea_trace] t.shape={t.shape}, data.shape={data.shape}, "
...
def plot_spike_train(result: SpikeTrain, ctx: PipelineContext, last_op) -> None:
    ...
    plt.title(
        f"Source {result.source_id}: {mode_label} Thresholding ({result.num_spikes} spikes)"
...
def plot_spike_pca(result: SpikePCA, ctx: PipelineContext, last_op) -> None:
    ...
    if len(result.pca_components) == 0 or n_spikes == 0:
        ...
        plt.text(
            0.5,
            0.5,
            f"Too few spikes for PCA (n={n_spikes})",
            ha="center",
            va="center",
            fontsize=14,
        ...
        plt.title(
            f"Source {result.source_id}: PCA Waveform Space"
    ...
    if proj.shape[1] >= 2:
        ...
        if result.n_outliers > 0:
            ax1.scatter(
                proj[result.outlier_mask, 0],
                proj[result.outlier_mask, 1],
                c="red",
                marker="x",
                s=50,
                linewidths=1.5,
    ...
    if inlier.any():
        ax2.plot(t_ms, result.waveforms[inlier].T, color="gray", alpha=0.15, linewidth=0.5)
    if result.n_outliers > 0:
        ax2.plot(
            t_ms, result.waveforms[result.outlier_mask].T, color="red", alpha=0.4, linewidth=0.5
        )
    if inlier.any():
...
def plot_correlation(result: CorrelationResult, ctx: PipelineContext, last_op) -> None:
    ...
    plt.scatter(
        result.x_coords[result.best_idx],
        result.y_coords[result.best_idx],
        marker="*",
        s=500,
        c=[best_color],
        edgecolors="black",
    ...
    plt.title(
        f"Spatial Correlation Map: Ca ROI {result.ca_trace_idx}\n"
...
def plot_spectrogram(result: Spectrogram, ctx: PipelineContext, last_op) -> None:
    ...
    im = plt.pcolormesh(
        result.times, result.frequencies, result.power, shading="auto", cmap="viridis"
    )
...
def plot_overlay(results, labels, ctx: PipelineContext) -> None:
    ...
    for i, (result, label) in enumerate(zip(results, labels)):
        ...
        plt.plot(
            t[:min_len], d_norm, linewidth=1, alpha=0.7, color=colors[i % len(colors)], label=label
        )

    plt.xlabel("Time (ms)")
    plt.ylabel("Normalized Amplitude (0-1)")
    plt.title(
        "Overlay: Signals normalized to [0, 1] for visual comparison" + _window_suffix(ws, fs)
    )
    plt.legend(loc="upper right")
    plt.grid(True, alpha=0.3)
    plt.gca().margins(x=0)
