"""Project workspace management for CASI.
...
The active project is tracked via ``.casi/active_project`` (an untracked
file at the repo root containing a single project name). All ``casi run``
...
from casi.config import find_project_root

# -- constants --------------------------------------------------------------

PROJECTS_DIRNAME = "projects"
TEMPLATE_NAME = "TEMPLATE"
ACTIVE_POINTER_REL = Path(".casi") / "active_project"
...
class ProjectInfo:
    """Summary row for ``casi project list``."""
    name: str
    path: Path
...
def project_root() -> Path:
    """Return the absolute path of ``projects/`` under the CASI repo root.

    Walks up from CWD looking for ``pyproject.toml`` (delegating to
    :func:`casi.config.find_project_root`). The projects directory is
...
def active_project_path() -> Path:
    """Return the absolute path to ``.casi/active_project`` (may not exist)."""
...
def resolve_active_project() -> Optional[Path]:
    ...
    Reads ``.casi/active_project`` and validates the named project still
    ...
    if not candidate.is_dir():
        print(
            ...
            f"clearing pointer with `casi project close` is recommended.",
            file=sys.stderr,
        )
        return None
    return candidate
...
def open_project(name: str) -> Path:
    ...
    Creates ``.casi/`` if needed. Writes the bare project name (not the full
...
def get_project_config(name_or_path: str | Path) -> dict:
    ...
    that is the job of ``casi.config.apply_project_overrides``.
    """
...
def _file_fingerprints_for_paths(paths_cfg: dict) -> dict[str, dict]:
    ...
    Mirrors the helper in ``casi.sessions`` without importing it (avoids
    the heavy ``casi.engine`` import chain when all we want is a fingerprint).
