"""``casi`` command-line interface for CASI.
...
wall between the conversation layer and the computation layer:
    ...
    casi config show                         # print loaded configuration
    casi config validate                     # check that referenced files exist
    casi config edit globals plot_backend pyqtgraph
    casi ops list                            # list all operations + parameter schemas
    casi sources list                        # list registered data sources
    casi project new my-analysis
    casi project open my-analysis
    casi project list
    casi session new --label "test-b"
    casi session list
    casi session show <session_dir>
    casi run "mea_trace(861).butter_bandpass.spectrogram"  --session <session_dir>

``mea_trace`` in the DSL refers to the multi-electrode-array hardware data
source and is unrelated to the old CLI name. Run ``casi --help`` for the
...
from casi import __version__ as CASI_VERSION
from casi.config import (
    DEFAULT_CONFIG_DIR,
...
from casi import projects as _projects
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="casi",
        description="CASI: Calcium-Assisted Spike Identity - pipeline CLI.",
    )
    parser.add_argument("--version", action="version", version=f"casi {CASI_VERSION}")
    parser.add_argument(
        "--config-dir",
    ...
    # ----- project -----
    p_project = sub.add_parser("project", help="Manage CASI project workspaces")
    ...
    p_proj_new.add_argument("--open", action="store_true",
                            help="Also set the new project as active")
    ...
    p_proj_info.add_argument("name", nargs="?", default=None,
    ...
    p_proj_ref = sub_project.add_parser(
        "reference", help="Manage references stored in a project"
    )
    ...
    p_ref_add.add_argument(
        "--source", required=True, choices=("web", "user", "claude"),
    ...
    p_find_plot.add_argument("dsl", help='DSL expression, e.g. "mea_trace(861).butter_bandpass.spectrogram"')
    p_find_plot.add_argument("--window", default=None,
                             help='Window directive: "full" or "<start_ms>,<end_ms>"')
    ...
    p_run.add_argument("dsl", help='DSL expression, e.g. "mea_trace(861).butter_bandpass.spectrogram"')
    ...
    p_run.add_argument("--label", default=None, help="Label for a new session (with --session omitted)")
    p_run.add_argument("--window", default=None,
                       help='Window directive: "full" or "<start_ms>,<end_ms>" '
                            '(overrides globals.yaml)')
    p_run.add_argument("--backend", default=None,
                       help="Override globals.yaml plot_backend for this run")
    p_run.add_argument("--force", action="store_true",
                       help="Bypass the project plot cache and always re-run")
...
def cmd_ops_list(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        for op_name, transitions in TYPE_TRANSITIONS.items():
            out[op_name] = {
                "transitions": {
                    in_type.__name__: out_type.__name__
    ...
    for op_name in sorted(TYPE_TRANSITIONS):
        ...
        if transitions:
            tr = "  ".join(
                f"{in_t.__name__}->{out_t.__name__}"
...
def _effective_window(
    args_window: str | None, globals_cfg: dict,
) -> str | list | None:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    from casi.sessions import new_session, write_manifest
    ...
    if project_dir is None:
        ...
        print("  casi project new <name> --open", file=sys.stderr)
        print("  casi project open <name>", file=sys.stderr)
        return 1
    ...
    if not getattr(args, "force", False):
        try:
            ...
            print(f"error: invalid --window {args.window!r}; expected 'full' or 'start,end'",
                  file=sys.stderr)
            return 1
    ...
    write_manifest(session_dir, ctx=_StubCtx(), paths_cfg=paths_for_run,
                   ops_cfg=cfg.ops, globals_cfg=globals_for_run)
...
def cmd_project_info(args: argparse.Namespace) -> int:
    ...
    if name is None:
        active = _projects.resolve_active_project()
        if active is None:
            print("no active project; pass a project name or `casi project open <name>`",
                  file=sys.stderr)
            return 1
        name = active.name
...
def _resolve_project_arg(name: str | None) -> Path | None:
    ...
    if active is None:
        print(
            "no active project; pass --project <name> or `casi project open <name>`",
            file=sys.stderr,
        )
        return None
    return active
