...
The CASI pipeline is configured via three YAML files instead of inline Python
...
def load_configs(config_dir: Path | str = DEFAULT_CONFIG_DIR) -> CasiConfig:
    """Load and lightly validate paths.yaml + ops.yaml + globals.yaml.

    Returns a ``CasiConfig`` with paths expanded to absolute strings. Missing
...
def render_summary(cfg: CasiConfig) -> str:
    """Human-readable one-screen summary used by `casi config show` and the agent."""
    lines: list[str] = []
    lines.append(f"CASI configuration  (project root: {cfg.project_root})")
    lines.append("")
...
def apply_project_overrides(cfg: CasiConfig, project_dir: Path) -> CasiConfig:
    ...
    Returns a new ``CasiConfig`` (does not mutate the input).
    """
