...
src/iris/          Python package — ops engine, DSL, config, projects, sessions
  daemon/            FastAPI backend (port 4002) — runs ops, profiles data
...
| Python ops engine / DSL / pipeline | [src/iris/CLAUDE.md](src/iris/CLAUDE.md) → package layout + op registry |
...
- **The webapp** (`iris-app/`) is the primary interface. The Express server wraps the Claude Code SDK to give the AI access to the filesystem, Python engine, and project context.
...
The `iris` CLI remains functional for direct use:
...
- Run `cd iris-app && npm run dev` to start the dev server.
